...
export function VehiclePassabilityModal({
  ...
  const [vehicles, setVehicles] = useState<VehicleClearanceCategory[]>(DEFAULT_VEHICLE_CLEARANCES);
  const [selectedDepth, setSelectedDepth] = useState<number>(35); // in cm

  useEffect(() => {
    fetchApi<any>('/config/vehicles')
      .then((res) => {
    ...
    if (socket) {
      const handleUpdate = (updated: VehicleClearanceCategory[]) => {
        if (Array.isArray(updated) && updated.length > 0) {
          setVehicles(updated);
        }
      };
      socket.on('vehicles:updated', handleUpdate);
      return () => {
        socket.off('vehicles:updated', handleUpdate);
      };
    }
  }, []);
  ...
  const getPassabilityStatus = (v: VehicleClearanceCategory) => {
